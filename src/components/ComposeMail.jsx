...
const ComposeMail = () => {
    ...
    const submitHandler = async (e) => {
        e.preventDefault();
        await addDoc(collection(db, "emailsend"), {
    ...
    return (
        <div className={`${open ? 'block' : 'hidden'} bg-white max-w-6xl shadow-xl shadow-slate-600 rounded-t-md`}>
            ...
            <form onSubmit={submitHandler} className='flex flex-col p-3 gap-2'>
                <input onChange={changeEventHandler} name="recipients" value={formData.recipients} type="text" placeholder='To' className='outline-none py-1' />
